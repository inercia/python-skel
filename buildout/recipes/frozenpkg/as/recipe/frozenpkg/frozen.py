...
class Frozen(object):
    ...
    def _getPythonVers (self, pythonexe):
        if os.path.exists(pythonexe):
            ...
            # self._log('Running %s' % (" ".join(pythonquery)))
            job = subprocess.Popen(pythonquery, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
            (stdoutdata, stderrdata) = job.communicate()
            if job.returncode == 0:
    ...
    def _copyPythonDist (self, root_dir, install_prefix):
        ...
        self._log(
            'Copying python binary %s to %s' % (self.python_bin, self.install_prefix + "/bin"))
        ...
        if not os.path.exists(venv_lib_sdir):
            ...
            venv_lib_sdir = os.path.abspath(
                self.buildout['buildout']['directory'] + venv_lib_prefix)
                        ...
                        for k, v in namespaces.iteritems():
                            ...
                            if len(v) > 0:
                                ...
                                if not os.path.isdir(egg_ns_dir):
                                    self.logger.info(
                                        "WARNING: while processing egg '%s': package '%s' is zipped. Skipping." % (
                        ...
                        if package_name in namespaces:
                            ...
                            if os.path.isdir(dist.location):
                                ...
                                if os.path.exists(package_location):
                                    pythonpath += copy_package(package_location, package_dest,
                                                               package_name)
                                ...
                                package_location = os.path.join(dist.location,
                                                                package_name + ".dll")
    ...
    def _copyAuxFiles (self, root_dir, install_prefix):
        ...
        activate_this_src = os.path.join(self.buildout['buildout']['bin-directory'],
                                         "activate_this.py")
